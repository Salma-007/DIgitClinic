...
<head>
    ...
    <style>
        ...
        * {
        ...
        .stat-card.docteurs { border-top: 4px solid #3498db; }
        .stat-card.actifs { border-top: 4px solid #27ae60; }
        .stat-card.specialites { border-top: 4px solid #e74c3c; }
        ...
        .badge-orange { background: #ffeaa7; color: #e17055; }
    ...
    <main class="main-content">
        ...
        <span class="stat-number">
          <c:choose>
              ...
              <c:otherwise>0</c:otherwise>
                ...
                <i class="fas fa-user-md" style="font-size: 2rem; color: #3498db; margin-top: 10px;"></i>
            </div>
            <div class="stat-card actifs">
        <span class="stat-number">
          <c:choose>
              <c:when test="${not empty stats.docteursActifs}">${stats.docteursActifs}</c:when>
              <c:otherwise>0</c:otherwise>
          </c:choose>
        </span>
        ...
        <span class="stat-number">
          <c:choose>
              ...
              <c:otherwise>0</c:otherwise>
                ...
                <i class="fas fa-stethoscope" style="font-size: 2rem; color: #e74c3c; margin-top: 10px;"></i>
            </div>
        ...
        <div class="content-card">
            <div class="table-header">
                ...
                <a href="docteurs?action=new" class="btn btn-primary" style="margin-top: 10px;">
            ...
            <c:if test="${empty docteurs}">
                <div style="text-align: center; padding: 40px; color: #7f8c8d;">
                    <i class="fas fa-user-md" style="font-size: 3rem; margin-bottom: 20px;"></i>
                    <p>Aucun médecin trouvé dans la base de données.</p>
                    <a href="docteurs?action=new" class="btn btn-primary" style="margin-top: 10px;">
                        <i class="fas fa-plus"></i> Créer le premier médecin
                    </a>
                </div>
            </c:if>
            ...
            <c:if test="${not empty docteurs}">
                        ...
                        <tr>
                            <td>
                                <div class="docteur-info">
                                    <div class="docteur-avatar" style="background:
                                    <c:choose>
                                    <c:when test="${docteur.specialite.contains('Cardio')}">#e74c3c</c:when>
                                    <c:when test="${docteur.specialite.contains('Dermato')}">#3498db</c:when>
                                    <c:when test="${docteur.specialite.contains('Pédiatrie')}">#f39c12</c:when>
                                    <c:when test="${docteur.specialite.contains('Neuro')}">#9b59b6</c:when>
                                    <c:when test="${docteur.specialite.contains('Ortho')}">#1abc9c</c:when>
                                    <c:otherwise>#7f8c8d</c:otherwise>
                                    </c:choose>;">
                                            ${docteur.prenom.charAt(0)}${docteur.nom.charAt(0)}
                                    </div>
                                    <div>
                                        <div class="docteur-name">Dr. ${docteur.prenom} ${docteur.nom}</div>
                                        <div class="docteur-specialite">${docteur.email}</div>
                                    </div>
                                </div>
                            </td>
                            <td>
                <span class="badge badge-purple">
                  <i class="fas fa-stethoscope"></i> ${docteur.specialite}
                </span>
                            </td>
                            <td>
                <span class="badge badge-primary">
                  <i class="fas fa-building"></i> ${docteur.departement.nom}
                </span>
                            </td>
                            <td>
                <span class="badge badge-orange">
                  ...
                  <c:choose>
                      <c:when test="${not empty docteur.planning}">${docteur.planning.size()}</c:when>
                      <c:otherwise>0</c:otherwise>
                  </c:choose>
                  consultations
                </span>
        ...
        <div class="content-card">
            <div class="table-header">
                <h3><i class="fas fa-chart-pie"></i> Répartition par Spécialité</h3>
            </div>
            <div style="text-align: center; padding: 20px; color: #7f8c8d;">
                <i class="fas fa-user-md" style="font-size: 2rem; margin-bottom: 10px;"></i>
                <p>Distribution des médecins par spécialité médicale</p>
                <div style="display: flex; justify-content: center; gap: 20px; margin-top: 20px; flex-wrap: wrap;">
                    <div style="text-align: center;">
                        ...
                        <div>Cardiologie</div>
                        <small>
                            <c:set var="cardioCount" value="0" />
                            <c:forEach var="docteur" items="${docteurs}">
                                <c:if test="${docteur.specialite.contains('Cardio')}">
                                    <c:set var="cardioCount" value="${cardioCount + 1}" />
                                </c:if>
                            </c:forEach>
                            ${cardioCount} médecins
                        </small>
                    </div>
                    <div style="text-align: center;">
                        ...
                        <div>Neurologie</div>
                        <small>
                            <c:set var="neuroCount" value="0" />
                            <c:forEach var="docteur" items="${docteurs}">
                                <c:if test="${docteur.specialite.contains('Neuro')}">
                                    <c:set var="neuroCount" value="${neuroCount + 1}" />
                                </c:if>
                            </c:forEach>
                            ${neuroCount} médecins
                        </small>
                    ...
                    <div style="text-align: center;">
                        ...
                        <div>Pédiatrie</div>
                        <small>
                            <c:set var="pediatrieCount" value="0" />
                            <c:forEach var="docteur" items="${docteurs}">
                                <c:if test="${docteur.specialite.contains('Pédiatrie')}">
                                    <c:set var="pediatrieCount" value="${pediatrieCount + 1}" />
                                </c:if>
                            </c:forEach>
                            ${pediatrieCount} médecins
                        </small>
                    </div>
                </div>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        navLinks.forEach(link => {
            ...
            if (linkPage === currentPage || (currentPage === '' && linkPage === 'docteurs')) {
